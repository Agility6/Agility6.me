  ...
  return (tree) => {
    visit(tree, 'element', (node, index, parent) => {
      ...
      for (const imgNode of imgNodes) {
        ...
        const width = Number(imgNode.properties?.width)
        const height = Number(imgNode.properties?.height)
        const maxImageWidth = 1600

        if (Number.isFinite(width) && Number.isFinite(height) && width > maxImageWidth) {
          const scale = maxImageWidth / width
          imgNode.properties.width = maxImageWidth
          imgNode.properties.height = Math.round(height * scale)
        }
