...
第一部分 `Agent1`  已经生成了以下文件
- `architecture.md`
- `roadmap.md`

大致看一下这些文件内容，architecture是原项目的架构图信息，以及核心流程、算法等介绍。`roadmap`则是根据原项目的信息，生成的最佳实践路线，可以看到这里一共分为10个步骤来完成，每一个步骤都有对应的目标、细化步骤、完成标准。

第二部分 `Agent2` Rust导师上线，只需要给定对应的任务，就能根据对应的知识点生成具体的 `guide.md` 。包括对原代码的解析、使用Rust实现这段逻辑应该掌握什么知识点，完整的Rust代码实现。

![img](./_assets/codex-learn-rust/1.png)
