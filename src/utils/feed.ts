...
const imagesGlob = import.meta.glob<{ default: ImageMetadata }>(
  '/src/content/posts/_assets/**/*.{jpeg,jpg,png,gif,webp,JPEG,JPG,PNG,GIF,WEBP}'
)
  ...
  for (const img of imageTags) {
    ...
    if (normalizedSrc.startsWith('./') || normalizedSrc.startsWith('../')) {
      ...
      if (normalizedSrc.startsWith('./')) {
        // ./xxx -> postDir/xxx
        resolvedPath = path.posix.join('/src/content/posts', postDir, normalizedSrc.slice(2))
      } else {
        // ../xxx -> Resolve to parent directory
        resolvedPath = path.posix.resolve('/src/content/posts', postDir, normalizedSrc)
      }
      ...
      if (imagesGlob[resolvedPath]) {
        try {
          ...
          console.error(`[Feed] Image processing failed: ${normalizedSrc} -> ${resolvedPath}`, error)
      ...
      img.setAttribute('src', new URL(normalizedSrc, baseUrl).toString())
    }
  }
